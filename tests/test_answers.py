def test_submit_and_get_public_answers(client, mock_current_user):
    ...
    assert res1.json()["nickname"] == mock_current_user.name
    ...
    assert res2.json()["nickname"] == mock_current_user.name
...
def test_submit_public_answer_unauthorized(client):
    """비로그인 상태에서 공개 답변 등록 시 401 차단 테스트"""
    enter_res = client.post("/api/books/enter", json={"title": "1984", "author": "조지 오웰"})
    book_id = enter_res.json()["book"]["id"]
    questions = client.get(f"/api/books/{book_id}/questions").json()
    question_id = questions[0]["id"]

    res = client.post(f"/api/questions/{question_id}/answers", json={"answer": "답변"})
